...
void FtlImpl_Fast::initialize_log_pages()
{
	...
	Event event = Event(WRITE, 1, 1, 0, 'a');
	// RW
...
void FtlImpl_Fast::merge_sequential(Event &event)
{
	...
	for (uint i=0;i<BLOCK_SIZE;i++)
	{
		...
		Event readEvent = Event(READ, event.get_logical_address(), 1, event.get_start_time(), 'a');
		readEvent.set_address(readAddress);
		if (controller.issue(readEvent) == FAILURE) { printf("Read failed\n"); return; }

		Event writeEvent = Event(WRITE, event.get_logical_address(), 1, event.get_start_time()+readEvent.get_time_taken(), 'a');
...
{
	Event writeEvent = Event(WRITE, event.get_logical_address(), 1, event.get_start_time(), 'a');
